...
Cpu_count = Fun_Get_CPU_Count(args.Recommended_Cpu_count,LogFile)
...
Mem = Fun_Get_Memory(args.Recommended_Memory,LogFile)
if Mem[0]==False:
...
Storage=Fun_GetFreeDisk(args.DriveName,args.Recommended_Disk_Space)
if Storage[1]==1:
	f.write('Check_Disk_Space: Fail ')
	if Storage[0]=='DNF':
		f.write('Directory '+args.DriveName+ ' Not found'+"\n")
	else:
		f.write("Recommended Value is "+ str(args.Recommended_Disk_Space)+ " , Actual Value is "+args(Storage[0])+"\n" )
elif Storage[1]==0:
	f.write('Check_Disk_Space: Pass ')
	f.write("Recommended Value is "+str(args.Recommended_Disk_Space)+" , Actual Value is "+str(Storage[0])+"\n" )
